...
namespace MasterYi
{
    internal class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Config.SubMenu("qconfig")
                .AddItem(new MenuItem("smartQloseaggro", "Lose turret aggro with Q").SetValue(true));
            Config.SubMenu("qconfig")
                .AddItem(new MenuItem("smartQrunaway", "Use Q if enemy is running away").SetValue(true));
            ...
            //Config.SubMenu("flee").AddItem(new MenuItem("fleeKey", "Flee key").
            ...
            Config.SubMenu("misc")
                .AddItem(new MenuItem("AutoSmite", "Auto Smite").SetValue(new KeyBind('J', KeyBindType.Toggle)));
            Config.SubMenu("misc")
                .AddItem(
        ...
        private static void KillSteal()
        {
            ...
            if (IgniteSlot != SpellSlot.Unknown &&
                ...
                target.Health <= GetSpellDamage(target, IgniteSlot))
            {
        ...
        private static void AutoSmite()
        {
            ...
            if (SmiteSlot == SpellSlot.Unknown)
            {
                if (Config.Item("AutoSmite").GetValue<KeyBind>().Active)
                {
                    ...
                    var firstOrDefault =
                        ObjectManager.Player.Spellbook.Spells.FirstOrDefault(spell => spell.Name.Contains("mite"));
                    ...
                    var vMonsters = MinionManager.GetMinions(
                        ObjectManager.Player.ServerPosition, firstOrDefault.SData.CastRange[0], MinionTypes.All,
                        MinionTeam.NotAlly, MinionOrderTypes.Health);
                    foreach (
                        var vMonster in
                            vMonsters.Where(
                                vMonster =>
                                    vMonster != null && !vMonster.IsDead && !ObjectManager.Player.IsDead &&
                                    !ObjectManager.Player.IsStunned && SmiteSlot != SpellSlot.Unknown &&
                                    ObjectManager.Player.Spellbook.CanUseSpell(SmiteSlot) == SpellState.Ready)
                                .Where(
                                    vMonster =>
                                        (vMonster.Health <
                                         ObjectManager.Player.GetSummonerSpellDamage(
                                             vMonster, Damage.SummonerSpell.Smite)) &&
